...
// Normaliza CSV removendo BOM e aplicando encoding fixo
const normalizeCsv = (csvData) =>
  normalizeEncodingBlock((csvData && typeof csvData === 'string' ? csvData : '').replace(/^\uFEFF/, ''));

// Limpa valor individual aplicando correção de encoding
const cleanField = (value, options = {}) =>
  typeof value === 'string' ? fixStringEncoding(value.trim(), options) : value;

// Importação de distritos
export const importDistritos = async (csvData) => {
  // Limpar encoding do CSV antes de processar
  const cleanedCSV = normalizeCsv(csvData);
  ...
  for (const line of lines) {
    ...
    const codigo = cleanField(codigoRaw);
    const nome = cleanField(nomeRaw, { dictionary: 'districts' });
...
export const importConcelhos = async (csvData) => {
  ...
  for (const line of lines) {
    const [codigoConcelho, codigoDistrito, nome] = line.split(',').map(v => cleanField(v.trim()));
...
export const importCodigosPostais = async (csvData) => {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    const localidade = cleanField(localidadeRaw);
...
export const importTransportes = async (csvData) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const parts = line.split(',').map(v => cleanField(v.trim()));
