...
import { ARGON2_CONFIG, validatePassword, validateName, isValidEmail } from '../utils/validation.js';
...
export const importUsersFromArray = async (usersArray) => {
  ...
  for (const userData of usersArray) {
    try {
      ...
      // Detectar se password já está hasheada (Argon2)
      const isAlreadyHashed = userData.password.startsWith('$argon2');
      const passwordToUse = isAlreadyHashed
        ? userData.password
        : await argon2.hash(userData.password, ARGON2_CONFIG);
