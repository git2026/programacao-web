...
export const register = async (req, res) => {
  try {
    ...
    if (!email && !password && !name) {
      return sendError(res, 400, 'Email, password e nome são obrigatórios');
    } else if (!email) {
      return sendError(res, 400, 'Email é obrigatório');
    } else if (!password) {
      return sendError(res, 400, 'Password é obrigatória');
    } else if (!name) {
    ...
    if (!isValidEmail(email)) {
      return sendError(res, 400, 'Formato de email inválido. Exemplo: utilizador@exemplo.com');
    }
    ...
    if (role && !isValidRole(role)) {
      return sendError(res, 400, 'Cargo inválido. Valores permitidos: admin, editor, guest');
    }
...
export const login = async (req, res) => {
  try {
    ...
    if (!email || !password) {
      if (!email && !password) {
        return sendError(res, 400, 'Email e password são obrigatórios');
      } else if (!email) {
        return sendError(res, 400, 'Email é obrigatório');
      } else {
        return sendError(res, 400, 'Password é obrigatória');
