...
// Argon2: configuração máxima de segurança (recomendado pelo OWASP)
// Máxima segurança: memoryCost alto, timeCost alto, parallelism otimizado
const ARGON2_CONFIG = {
  type: 1, // Argon2i (resistente a side-channel attacks)
  memoryCost: 131072, // 128 MB (máxima segurança)
  timeCost: 5, // 5 iterações (máxima segurança)
...
export const validateName = (name) => {
  ...
  // Verificar eventos JavaScript e protocolos perigosos (proteção adicional)
...
// Sanitizar string (prevenir XSS básico) - método antigo mantido para compatibilidade
export const sanitizeString = (str) => {
  if (!str || typeof str !== 'string') return '';
  return str.trim().replace(/[<>\"']/g, '');
};

// Sanitizar HTML (remove todas as tags e caracteres perigosos)
...
export const isValidUrl = (url) => {
  ...
  if (trimmedUrl.length === 0) return true; // URL vazia é válida (opcional)
  ...
  if (!trimmedUrl.match(/^https?:\/\//i)) {
    return false; // Exigir protocolo explícito
  }
...
export const validateAndSanitizeImagePath = (imagePath) => {
  if (!imagePath || typeof imagePath !== 'string') {
    return { valid: true, sanitized: '' }; // Imagem opcional
  }
...
export { ARGON2_CONFIG, MIN_PASSWORD_LENGTH, MAX_PASSWORD_LENGTH, MIN_NAME_LENGTH };
